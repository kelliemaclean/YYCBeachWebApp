...
# Example view function to display the form inputs
def generate_schedule(request):
    if request.method == 'POST':
        # Assuming you're getting inputs like 'teams', 'weeks', etc.
        teams = int(request.POST.get('teams'))
        weeks = int(request.POST.get('weeks'))
        courts = int(request.POST.get('courts'))
        bye = request.POST.get('bye')
        filename = request.POST.get('filename')
        # Add other inputs as needed

        # Pass the inputs back to the template
        context = {
            'teams': teams,
            'weeks': weeks,
            'courts': courts,
        ...
        schedule = main(num_teams = teams, num_weeks = weeks, num_courts = courts, bye = bye)
        schedule = schedule.to_html(index=False, classes='table table-striped')
        return render(request, 'generate_schedule.html', {'schedule': schedule})
    else:
        # For GET request, just render the form without any context
